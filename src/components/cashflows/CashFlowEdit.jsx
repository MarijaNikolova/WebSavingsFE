...
const CashFlowEdit = (props) => {
  const { cashflow, handleEditModeChange, handleRefetch } = props;
  ...
  return (
    <FinalForm
      onSubmit={(values) => {
        console.log(values);
        console.log('editing cashflow with id', cashflow.id);
        put(ENDPOINT_URL + 'cashflows', { customerId }, { id: cashflow.id, ...values })
          .then((result) => {
      ...
      render={({ handleSubmit }) => (
        <form onSubmit={handleSubmit}>
          <Card.Title>Edit cash flow</Card.Title>
          <StyledAccountEditFormContainer>
            <StyledAccountFormColumn>
              <div>
